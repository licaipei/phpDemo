...
class cpTemplate {
	...
	private function _insert_scripts($args,$type)
	{
		$scripts = array();
		...
		foreach ($arr AS $val)
		{
			if (in_array($val, $scripts) == false)
			{
				$scripts[] = $val;
				if($type==1){
					if($val{0} == '/') $str .= '<link href="' . str_replace('[root]', __ROOT__, substr($val, 1)) . '" rel="stylesheet" type="text/css" />';
					else $str .= '<link href="'.__PUBLIC__.'/css/' . $val . '" rel="stylesheet" type="text/css" />';
				}else{
					if($val{0} == '/') $str .= '<script type="text/javascript" src="' . str_replace('[root]', __ROOT__, substr($val, 1)) . '"></script>';
