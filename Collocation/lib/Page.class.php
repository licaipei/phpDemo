...
class Page
{
	...
	public function nowBar($style='',$nowIndex_style='active')
	{
		...
		for($i=$begin;$i<$begin+$this->pageBarNum;$i++)
		{
			if($i<=$this->totalPage)
			{
				...
				$return.=$this->_getText('<li class="'.$nowIndex_style.'"><a herf="">'.$i.'</a></li>');
			}
			else
			{
				break;
	...
	function _getLink($url,$text,$style='')
	{
		...
		{
			//如果是使用AJAX模式
			return '<li class="paginItem"><a '.$style.' href="javascript:'.$this->ajaxActionName.'(\''.$url.'\')">'.$text.'</a></li>';
		}
		else
		{
			return '<li class="paginItem"><a '.$style.' href="'.$url.'">'.$text.'</a></li>';
		}
	}
}
?>
