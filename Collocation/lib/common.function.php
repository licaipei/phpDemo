...
//检查是否是正确的电话号码，是则返回true，否则返回false
function is_mobile($mobilephone){
	if(preg_match("/^13[0-9]{1}[0-9]{8}$|14[57]{1}[0-9]{8}$|15[012356789]{1}[0-9]{8}$|18[012356789]{1}[0-9]{8}$/i", $mobilephone)){
        return true;
    }else{
        return false;
    }
...
function cpright(){
     if (defined('__CRAWLER__') && __CRAWLER__) echo cp_decode('');
}
//获取微秒时间，常用于计算程序的运行时间
function utime(){
...
function cp_authcode($string, $operation = 'DECODE', $key = '', $expiry = 0) {

	$ckey_length = 8;	//note 随机密钥长度 取值 0-32;
				//note 加入随机密钥，可以令密文无任何规律，即便是原文和密钥完全相同，加密结果也会每次不同，增大破解难度。
				//note 取值越大，密文变动规律越大，密文变化 = 16 的 $ckey_length 次方
				//note 当此值为 0 时，则不产生随机密钥

	$key = md5($key ? $key : HC_KEY);
 ...
 * 引入样式/脚本
 * @param $args 包含文件“,”分开
 * @param $dir 目录相对于template,默认为为空
 * @param $type 默认为js
 */
function insert_scripts($args,$dir='',$type='js')
{
	$scripts = array();
	...
	foreach ($arr AS $val)
	{
		if (in_array($val, $scripts) == false)
		{
			$scripts[] = $val;
			if($type=='css'){
				if($val{0} == '/') $str .= '<link href="' . __ROOT__ . '/template' . $dir . '/css' . $val . '" rel="stylesheet" type="text/css" />';
				else $str .= '<link href="'.__PUBLIC__.'/css/' . $val . '" rel="stylesheet" type="text/css" />';
			}else{
				if($val{0} == '/') $str .= '<script type="text/javascript" src="' . __ROOT__ . '/template' . $dir . '/js' . $val . '"></script>';
				else $str .= '<script type="text/javascript" src="'.__PUBLIC__.'/js/' . $val . '"></script>';
			}
		}
	}
	return $str;
...
function holdersize($hold,$holdersize=0){
	if (is_dir($hold)&&@$handle = opendir($hold)) {
		...
		}return intval($holdersize/1024);
	}else return 0;
}
//打印多级菜单的方法
function getAllMenuShow($data, $pid=0)
{
	$ret = array();
	if(!empty($data)){
	foreach ($data as $k => $v) {
		if($v['parent_id'] == $pid) {
			$v['children'] = getAllMenuShow($data, $v['menu_id']);
			$ret[] = $v;
		}
	}
	}
