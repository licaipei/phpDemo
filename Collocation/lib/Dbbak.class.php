 ...
 */
	 ...
	 {
		$dbbackname=time();//文件夹名、文件前缀
		$this->folder($this->dataDir.$dbbackname);//创建文件夹
		$dir=$this->dataDir.$dbbackname.'/';//存放路径
     	...
     	if($subsection == 0)
		{
			...
			else
			{
     		...
     		$fileName = $dir.$dbbackname.'_all.sql.php';
		...
		else
		{
			...
			{
     			$fileName = $dir.$dbbackname.'_part'.$i.'.sql.php';
